...
def plot_data_example(train_x, train_y, test_x, test_y):
    ...
    # xticks from 0 to 6 pi
    #plt.xticks([0, np.pi, 2*np.pi, 3*np.pi, 4*np.pi, 5*np.pi, 6*np.pi], ['0', '$\pi$', '2$\pi$', '3$\pi$', '4$\pi$', '5$\pi$', '6$\pi$']) # E2
...
def plot_test_loss_vs_parameter(df, parameter, parameter_name, title, mse_val=None, colors=None, log=False, fig=None, ax=None, legend=False):
    # Calculate mean and standard deviation of test loss for each model and parameter
    ...
    TanhMLP_data = mean_std_df[mean_std_df['Model'] == 'TanhMLP']
    ...
    # Plot for each model with error bands
    for model_data, label in zip(
        [ReLUMLP_data, TanhMLP_data, SineMLP_data, SnakeMLP_data],
        ['ReLU', 'Tanh', 'Sine', 'Snake']
    ):
        ax.plot(model_data[parameter], model_data['Test_Mean_Loss'], '-o', label=label)
        ax.fill_between(
            model_data[parameter],
            model_data['Test_Mean_Loss'] - model_data['Test_Std_Loss'] / factor,
            model_data['Test_Mean_Loss'] + model_data['Test_Std_Loss'] / factor,
            alpha=0.2
        )

    # Plot horizontal and vertical lines if mse_val is provided
    if mse_val is not None:
        # Horizontal line
        ax.axhline(y=mse_val, color='black', linestyle='--', linewidth=1)
        ...
        for model_data, label in zip(
            ...
            # Find where the curve crosses the mse_val
            x_vals = model_data[parameter].values
            y_vals = model_data['Test_Mean_Loss'].values

            for i in range(len(y_vals) - 1):
                if (y_vals[i] >= mse_val >= y_vals[i + 1]) or (y_vals[i] <= mse_val <= y_vals[i + 1]):
                    # Linear interpolation to find the exact intersection point
                    x_cross = x_vals[i] + (x_vals[i + 1] - x_vals[i]) * (mse_val - y_vals[i]) / (y_vals[i + 1] - y_vals[i])
                    #ax.vlines(x=x_cross, ymin=1e-4, ymax=1e-2, color='grey', linestyle='--', linewidth=1)
                    ax.axvline(x=x_cross, color='grey', linestyle='--', linewidth=1)
                    #ax.plot(x_cross, mse_val, 'o')  # Mark the intersection point
