...
def main():
    ...
    # Data points
    idx_Xd = np.random.choice(X_star.shape[0], N_d, replace=False)
    X_data = X_star[idx_Xd]
    u_data = u_star[idx_Xd].squeeze()
    ...
    u_data_tensor = torch.tensor(u_data, dtype=torch.float64).view(-1, 1)
